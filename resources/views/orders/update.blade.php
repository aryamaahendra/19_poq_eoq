    ...
    <div class="w-full max-w-6xl mx-auto mt-6">
        <div class="breadcrumbs text-sm">
            <ul>
                <li><a>Transaksi</a></li>
                <li><a>Pembelian</a></li>
                <li>Ubah Pembelian</li>
            </ul>
        </div>
    </div>

    <div class="w-full max-w-6xl mx-auto my-6 main-card">
        <form action="{{ route('dshb.order.update', ['m_order' => $order->id]) }}" method="POST" class="card-body">
            @csrf
            @method('PUT')
            ...
            <div class="">
                ...
                <div class="grid grid-cols-2 gap-2 pb-8 mb-8 border-b">
                    ...
                    <x-forms.label-with-error name="date" label="Tanggal" required="{{ true }}">
                        <x-forms.input-text type="date" name="date" value="{{ old('date') ?? $order->date }}"
                            required />
                    </x-forms.label-with-error>

                    <x-forms.label-with-error name="from" label="Diterima Dari" required="{{ true }}">
                        <x-forms.input-text type="text" name="from" placeholder="CV. Kapan Lagi"
                            value="{{ old('from') ?? $order->from }}" required />
                    </x-forms.label-with-error>

                    <x-forms.label-with-error name="at" label="Di" required="{{ true }}">
