    ...
    <div class="w-full max-w-lg mx-auto my-6 main-card">
        <form action="{{ route('dshb.users.store') }}" method="POST" class="card-body">
            ...
            <x-forms.label-with-error name="email" label="Email" required="{{ true }}">
                <x-forms.input-text type="email" name="email" placeholder="email@aku.com" value="{{ old('email') }}"
                    required />
            </x-forms.label-with-error>

            <x-forms.label-with-error name="password" label="Password" required="{{ true }}">
                <x-forms.input-text type="password" name="password" placeholder="******" value="{{ old('password') }}"
                    required />
            </x-forms.label-with-error>

            <x-forms.label-with-error name="password_confirmation" label="Konfirmasi Password" value="{{ old('name') }}"
                required="{{ true }}">
                <x-forms.input-text type="password" name="password_confirmation" placeholder="******" required />
            </x-forms.label-with-error>
